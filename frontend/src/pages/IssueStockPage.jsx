...
export default function IssueStockPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await StockApi.issue(payload);
      const newQty = res?.data?.newQuantity;

      setOk(
        `Stock issued successfully.${typeof newQty === "number" ? ` New quantity: ${newQty}` : ""}`
      );
    ...
    <div>
      <div className="mb-4 flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-400">
          Remove stock from inventory and register a movement (Issue).
        </p>
      </div>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            />
            <div className="mt-2 text-xs text-slate-400">
              Must be greater than 0. Backend will prevent negative stock.
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="e.g. Sale / Internal usage"
            />
          </div>
        </div>
